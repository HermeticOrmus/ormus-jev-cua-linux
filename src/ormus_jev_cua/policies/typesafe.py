...
AT-SPI elements have stronger semantics than OCR elements, so prefer an accessibility target when both represent the same usable control.
...
class TypeSafeJevPolicy:
    ...
    def _build_questions(
        ...
        questions["hotkey_value"] = {
            ...
            "instructions": {"subtask": subtask.compact(), "rules": "Choose the hotkey if HOTKEY is selected. MOD means Ctrl on Linux (Cmd on macOS elsewhere)."},
        }
